...
Version gate (checked 2026-09-07): [RELION **5.1.0 is prerelease**](https://github.com/3dem/relion/releases/tag/5.1.0). It adds a picker based on the 4.75 Å Fourier signal, `amyprep`/`amyproc` Schemes, and an amyloid-specific retrained Blush network. The historical 5.0.0 capture already contains an `--amyloid` option; that flag's presence does not establish these new algorithms, Schemes, or model weights. Choose the [stable helical tutorial](https://relion.readthedocs.io/en/release-5.0/Reference/Helix.html) for 5.0.x and check the actual 5.1 binary, Scheme files, and network before using prerelease recipes. Validate picks and filament continuity before extraction; these 5.1 changes have not been locally run by this skill refresh.
