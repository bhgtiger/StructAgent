...
Writes:
  ...
  <out>.json        sidecar: {"ok": bool, "params": {...}, "stats": {...}, "error"?}
                    For --out mask.mrc the sidecar is mask.mrc.json (literal
                    concatenation of out path + ".json").
...
blur + re-threshold trick, not `volume morphology`):
  ...
  open <target_map>                                                  -> #1
  open <model>                                                       -> #2
  molmap <selection> <resolution> gridSpacing p                      -> #3
  [volume threshold #3 minimum t set 0;
   volume threshold ... maximum 0 setMaximum 1]                      -> binarize
  [volume gaussian ... sDev <dilation_A>;
   volume threshold ... minimum 0.25 set 0; ... setMaximum 1]        -> dilate-via-blur
  [volume gaussian #X sDev <soft_A>]                                 -> soft edge
...
def main(argv):
    ...
    try:
        ...
        # 2. Open model -> #2
        run(session, f"open {_q(args.model)}")
        ...
        if soft and soft < 2.0 * apix:
            stats["soft_warning"] = (
                f"soft={soft:.2f} A is much smaller than the 5*resolution heuristic; "
                "pass --gsfsc-resolution or a larger --soft for production masks."
            )
