...
Writes:
  <out>             the .mrc complement mask, resampled onto the target map's grid
  <out>.json        sidecar: {"ok": bool, "params": ..., "stats": ..., "error"?}
                    For --out mask_sub.mrc the sidecar is mask_sub.mrc.json.
...
Logic:
  ...
  volume threshold #4 minimum 0 set 0    # clamp negatives (NOT setMinimum)
  (optional) volume gaussian sDev <soft>           # soft edge
  volume resample onGrid #1
  save out

Input shape note: `volume subtract` followed by re-blur on inputs that
already have soft edges yields `soft − soft → clamp → soft again`, which
produces an over-blurred / shape-shifted boundary rather than a clean
binary complement. For best results, produce `--full`/`--region` with
binary (hard-edge) inputs and let this script add the single soft edge,
or document the expected boundary trade-off when chaining soft inputs.
"""
...
def main(argv):
    ...
    try:
        ...
        run(session, f"open {_q(args.target_map)}")
        from chimerax.map.volume import Volume
        target = [m for m in session.models if isinstance(m, Volume)][0]
        apix = float(target.data.step[0])
        stats["apix"] = apix
        run(session, f"open {_q(args.full)}")
        run(session, f"open {_q(args.region)}")
        ...
        run(session, f"save {_q(out_abs)} #{cur}")
