...
|---|---:|---|---|---|
| v4.0 major UI/data model transition | v3 → v4.0+ | New interface, integrated Live, project locks, changed project/job output filename behavior, new data-management guide. | Treat v3 UI/forum instructions as suspect; prefer v4+ docs for project locks, Live access, job cards, table/tree views. | `reference/release_notes/markdown/v4.0.md` |
| Legacy web app removed | v4.4+ | Legacy web app that could be started via `cryosparcm start app_legacy` is no longer bundled. | Do not recommend legacy app fallback on v4.4+. | `reference/release_notes/markdown/v4.4.md` |
| CUDA/driver requirement shift | v4.4+ | CryoSPARC bundles CUDA Toolkit 11.8; separate CUDA Toolkit no longer required for worker app; NVIDIA driver 520.61.05+ required. | On driver/CUDA errors, check driver first; do not assume missing local CUDA toolkit is the root cause. | `reference/release_notes/markdown/v4.4.md` |
...
| Dynamic refinement masks | v5.0+ | New automatic mask generation method and “Use dynamic refinement mask” refinement parameter. | Mask/refinement advice differs from v4; check whether dynamic masks are enabled before recommending manual static-mask tuning. | `reference/release_notes/markdown/v5.0.md`, `20_masks.md` |
...
|---|---|---|
| “Open Live at `BASE_PORT + 6`” | Since v4.0, Live is integrated into the main web application at `BASE_PORT`; `liveapp` service no longer has the old separate access pattern. | Access Live through the main cryoSPARC interface. (`v4.0.md`) |
| “Start the legacy v3 web app to work around UI issues” | Legacy app is no longer bundled in v4.4+. | Use current v4/v5 UI paths; update stale screenshots. (`v4.4.md`) |
| “Install/fix local CUDA Toolkit on workers” | v4.4+ bundles CUDA Toolkit 11.8 for worker app; v5 uses CUDA 12.8. Driver compatibility is often the real cliff. | Check NVIDIA driver and GPU compute capability first. (`v4.4.md`, `v5.0.md`) |
| “Gctf is a normal visible job” | In v4.0, Gctf is legacy and hidden unless legacy jobs are shown. | Prefer Patch CTF unless specifically using legacy Gctf; enable legacy jobs if needed. (`v4.0.md`) |
| “Use Deep Picker Train/Inference for new picking work” | Deep Picker is deprecated/removed from v5 job creation. | Use Topaz or other current picking workflow for v5. (`v5.0.md`) |
| “Use v4 `cryosparcm cli` command forms in scripts” | v5 CLI is not compatible with v4 `cli` commands. | Branch scripts by version and update Live/CLI automation for v5. (`v5.0.md`) |
| “Live configuration profiles can be moved/downgraded freely” | v5 Live profiles are not backwards compatible and are not retained when downgrading to v4. | Export/document profiles before migration; rebuild profiles after downgrade. (`v5.0.md`) |
| “Manual static masks are the normal refinement default” | v5 introduced a new automatic dynamic refinement-mask method. | Check dynamic-mask parameter and version before tuning mask dilation/soft edge. (`v5.0.md`, `20_masks.md`) |
| “SSD cache bugs are site-specific; just disable cache” | Multiple cache fixes landed v4.2, v4.4, v4.5, v4.6, v5.0. | Check version-specific cache fixes before disabling cache permanently. (`v4.2.md`–`v5.0.md`, `24_disk_and_storage.md`) |
| “Forum workaround for v5.0.0 project detach validation is data corruption” | Several v5.0.0–v5.0.2 migration validation failures were fixed in v5.0.1/v5.0.3. | Upgrade to latest v5 patch before manual DB/project intervention. (`v5.0.md`) |
...
### Install / admin
- v4.4+: check NVIDIA driver 520.61.05+ before blaming missing CUDA Toolkit. (`v4.4.md`)
- v4.6: CentOS 7 is deprecated; do not plan long-lived production upgrades on CentOS 7. (`v4.6.md`)
- v5.0+: audit GLIBC 2.28+, OS family, NVIDIA driver 570.26+, and GPU compute capability 5.0–12.0 before upgrade. (`v5.0.md`)
- v5.0+: v4 `cryosparcm cli` script snippets may be wrong; prefer v5 CLI reference. (`v5.0.md`, `14_cli_admin.md`)
- v5.0+: `cryosparcm recover`, `worker connect`, `worker disconnect`, and `delete_output_result_files` expand admin automation surfaces. (`v5.0.md`)

### CryoSPARC Live
- v4.0 moved Live into the primary interface; separate old Live access advice is stale. (`v4.0.md`)
- v4.4–v4.6 changed Live storage, plotting, session management, lane behavior, data-management tab, and compaction/restoration recommendations. (`v4.4.md`, `v4.6.md`)
- v5.0 added auto-start/auto-pause, delay worker startup, multiple sessions with different raw-data directories, and workers-per-GPU. (`v5.0.md`, `25_cryosparc_live.md`)
- v5 Live configuration profiles are not backwards compatible. (`v5.0.md`)

### Import / EER / file handling
- v4.6.2 fixed Falcon C EER import failures. If a Falcon C EER import bug appears on older v4.6/v4.5, update first. (`v4.6.md`)
- v4.6 fixed Live exposure pixel size display with EER upsampling. (`v4.6.md`)
- v5 improved detection/reporting of truncated movie/micrograph/particle corruption. (`v5.0.md`)
...
- v4.0–v4.5 changed Topaz setup, error messages, project-level executable path, resource defaults, downsampling mode, and extraction-radius handling. (`v4.0.md`, `v4.5.md`)
- v4.6 adds Inspect Particle Picks auto-cluster for denoised micrographs. (`v4.6.md`)
- v5 removes Deep Picker Train/Inference from new job creation; previously-run jobs remain visible. (`v5.0.md`)
- v5.0.4/v5.0.5 fixed Topaz Cross Validation parameter issues and decimal-value launch failures. (`v5.0.md`)

### Motion / CTF / RBMC
- v4.4 introduced major RBMC improvements, empirical dose weights, and multi-class motion estimation. (`v4.4.md`)
- v4.5 fixed Patch Motion rerun failure from missing empirical dose weights and freezes in motion/CTF jobs after abnormal child termination. (`v4.5.md`)
- v5 added hot-pixel threshold to Patch Motion Correction and RBMC. (`v5.0.md`)
- CTFFIND4 updated in v4.3; Gctf is legacy from v4.0 onward. (`v4.3.md`, `v4.0.md`)

### Refinement / masks
- v4.0 NU refinement gained optional Ewald sphere correction. (`v4.0.md`)
- v4.4 made NU refinement faster and added batch-size/OOM-related behavior fixes. (`v4.4.md`)
- v4.5 refinement jobs gained cFSC summary plots at every iteration. (`v4.5.md`)
- v5 dynamic refinement mask changes mean mask advice must branch by version and parameter state. (`v5.0.md`, `20_masks.md`)

### Classification / heterogeneity
- v4.1 introduced 3DFlex beta; later releases fixed multiple 3DFlex/3DVA issues. (`v4.1.md`, `v4.3.md`, `v4.5.md`, `v4.6.md`)
- v4.5 fixed 3D Classification class re-ordering failure when classes >12 and focus-mask plotting offset. (`v4.5.md`)
- v5.0.1 fixed 3D Classification required-parameter validation after v5.0.0 upgrade. (`v5.0.md`)
- v4.4 3DFlex Reconstruction supports CTF aberrations and lower-RAM particle access via SSD cache/project directories. (`v4.4.md`)

### Storage / cache / data management
- v4.0 project locks changed attach/detach/delete failure modes. (`v4.0.md`, `17_error_lookup.md`)
- v4.2 fixed SSD free-space infinite hang and cache timeout retries. (`v4.2.md`)
- v4.4 introduced improved SSD cache experimentally; v4.5 enabled the new cache by default with distributed locking strategy; v4.6 improved cluster filesystem diagnostics; v5 added fallback for silent SSD copy failures. (`v4.4.md`–`v5.0.md`, `24_disk_and_storage.md`)
- v4.3 introduced Data Cleanup Tools and `cryosparcm compact`; v5 added more cleanup controls and per-result deletion endpoint. (`v4.3.md`, `v5.0.md`)
...
- `reference/release_notes/markdown/v4.0.md` — v4 interface, Live integration, project locks, Gctf legacy, NU Ewald, early v4 fixes.
- `reference/release_notes/markdown/v4.1.md` — 3DFlex beta, multi-select/restart, owner command restrictions, 3DVA/Live/Gctf fixes.
- `reference/release_notes/markdown/v4.2.md` — CUDA 11.8 support, SSD cache fixes, intermediate results control, 3D Classification CTF-field fix.
- `reference/release_notes/markdown/v4.3.md` — Data Cleanup Tools, Performance Benchmarking, Extensive Validation, `compact`, CTFFIND4 update, Topaz/3DFlex/Live fixes.
- `reference/release_notes/markdown/v4.4.md` — bundled CUDA 11.8/driver 520.61.05+, Workflows/Blueprints, RBMC improvements, 3DFlex Reconstruction, improved SSD cache, legacy app removal.
- `reference/release_notes/markdown/v4.5.md` — cFSC plots, Rebalance Orientations, improved SSD cache default, Topaz changes, many Live/classification/3DFlex/cache fixes.
- `reference/release_notes/markdown/v4.6.md` — CentOS 7 deprecation, high-performance I/O, job groups, auto-cluster, `.seg` upload, Live data-management deprecation, EER/SSD/3DFlex fixes.
- `reference/release_notes/markdown/v5.0.md` — v5 OS/GPU/driver/CLI cliffs, dynamic masks, Live run configuration, Deep Picker removal, project validation fixes, recovery/cleanup/admin endpoints.
- `17_error_lookup.md` — symptom-first cross-reference for launch, filesystem, SSD cache, project-lock, and v5 migration errors.
- `14_cli_admin.md` — operational CLI/admin context.
- `20_masks.md` — mask/refinement context, especially for dynamic-mask branching.
- `24_disk_and_storage.md` — cache/data-management context.
- `25_cryosparc_live.md` — Live workflow context.
