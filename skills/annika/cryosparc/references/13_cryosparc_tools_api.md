...
Mechanics that belong in adjacent pages are linked, not duplicated: GUI behavior in `official cryoSPARC documentation`, workflow templating in `official cryoSPARC documentation`, output-group internals in `official cryoSPARC documentation`, project/workspace shape in `official cryoSPARC documentation`, troubleshooting in `15_troubleshooting.md`, error strings in `17_error_lookup.md`.
...
- cryosparc-tools is an open-source Python library for **CryoSPARC v4.1+**, released first as Beta in v4.1, with use cases that explicitly include "Programmatically read and write exposure, particle and volume data", "Access project, workspace and job data", "Build and run jobs to orchestrate custom cryo-EM workflows" and "Extend CryoSPARC functionality with third-party software packages" (`official cryoSPARC documentation`, `public cryoSPARC release notes v4.1`).
- Parameter code names that drive scripted parameter setting can be **copied from the GUI's 'Inputs and Parameters' tab** when previewing a job — that is the canonical way to discover the exact `name` to use in a tools script (`public cryoSPARC release notes v4.4`).
...
- The full reference documentation for the library is hosted at `https://tools.cryosparc.com/` and source at `https://github.com/cryoem-uoft/cryosparc-tools` — both linked from the official guide (`official cryoSPARC documentation`).
...
| Input | Notes |
|---|---|
| Base URL / host + base port | The same URL used to reach CryoSPARC from a browser. Tools versions ≥ v5 require only the base port forwarded; older tools required `BASE_PORT + 2`, `+3`, `+5` (`official cryoSPARC documentation` and tools release context) |
...
- **Use one CryoSPARC user account per automation context** and respect the GUI permissions model — projects are shared explicitly, so a tools session can only see what its user can see (`official cryoSPARC documentation`).
...
- For network filesystems where listed UNIX permissions are misleading, v5.0 introduced `CRYOSPARC_CLI_SKIP_ACCESS_CHECK=true` to disable filesystem permission checks for `cryosparcm` and `cryosparcw` command-line arguments (`public cryoSPARC release notes v5.0`). Do not set this casually — it is an instance-level switch for known-broken NFS permission reporting.
...
3. Use the **Outputs tab / output-group sidebar** to identify the exact output group names and result names you'll need to connect or load (`official cryoSPARC documentation`, `official cryoSPARC documentation`).
...
|---|---|---|---|
| **Project** | `P<N>` (e.g. `P3`) | A project directory on disk; all jobs and intermediate/output data live inside it; strict isolation between projects | `official cryoSPARC documentation` |
...
| **Job** | `J<N>` (e.g. `J42`) | One processing unit with inputs, parameters, outputs, logs, on-disk directory | `official cryoSPARC documentation` |
| **Output group** | Named (e.g. `particles`, `volume`, `exposures`) | Bundle of related output results | `official cryoSPARC documentation` |
...
Reading job state from cryosparc-tools maps to the same surface the GUI exposes (`official cryoSPARC documentation`):
...
| 4. Set parameters | Set parameters individually using the **exact parameter code name copied from the GUI's Inputs and Parameters tab** (v4.4+, `public cryoSPARC release notes v4.4`) | Parameter set returns success; job is still `building` and has no build error |
| 5. Connect inputs | Wire output groups from parent jobs into the new job's input groups; for surgical replacement at the result level, use a low-level result connection that mirrors the GUI's Low Level Results Interface (`official cryoSPARC documentation`) | Each required input slot is populated; no red error markers; passthrough resolution looks sane |
...
- **Read the dashboard's Errors and Warnings.** The v5.0 dashboard surfaces these prominently (`official cryoSPARC documentation`); scripted runs should fetch and assert on the same data, not just the binary completed/failed split.
...
CryoSPARC ships a first-class **Workflows** feature (`official cryoSPARC documentation`, `official cryoSPARC documentation`). For repeatable end-to-end pipelines this is almost always a better choice than building the job graph from scratch in cryosparc-tools.
...
- The published GPCR automation Workflow is a worked example of an end-to-end repeat-target pipeline (`official cryoSPARC documentation`).
...
- **Keep provenance.** Workflows preserve the parent/child connections that the GUI uses for the related-jobs sidebar and the comparison view (`official cryoSPARC documentation`). Don't reach around the workflow to mutate connections after apply — re-clone if a recipe needs to change.
- **Be aware of cross-version edges.** v5.0 fixed an issue where connections between Workflow jobs would fail in some cases due to a connection edge case (`public cryoSPARC release notes v5.0`); if a workflow apply behaves oddly across versions, check the changelog before debugging.
...
| Goal | Path |
|---|---|
| Read a job's particle / exposure / volume metadata as a dataset | Load the relevant output group from the job (e.g. `particles`, `exposures`, `micrographs`); the returned dataset includes slot columns like `blob`, `ctf`, `location`, `alignments3D` etc. (`official cryoSPARC documentation`) |
...
| Read at a specific iteration | Specify the output version; final iteration is `F`. The GUI's Outputs tab also lets you download specific iterations of low-level results (e.g. `alignments3D` at iteration 2) (`official cryoSPARC documentation`) |
...
| Read event log / job log | Tools mirrors the GUI's Event Log + Log tabs; for archival, the GUI's "Download Job Event Log" produces a PDF (job-log-PDF generation is a v4.0+ feature) (`official cryoSPARC documentation`) |
| Read a job report (event log + system logs) for debugging | GUI feature, see `official cryoSPARC documentation`; same artifact can be downloaded over the network |
...
| Inherit unchanged slots from a parent | Use passthrough so the External Job only owns the slots it actually modifies, and downstream jobs still see the parent's `ctf`, `location`, etc. (`official cryoSPARC documentation`) |
| Replace exactly one slot of a downstream job's input | Use the Low Level Results Interface pattern — drag-equivalent at the API level — to swap a specific result (e.g. replace `blob` from a downsampled refinement with `blob` from the full-size Extract from Micrographs job) without touching the other slots (`official cryoSPARC documentation`) |

### Exporting

| Goal | Path |
|---|---|
| Share a single job's results with another user / instance | Export the job (GUI action; the exported `.cs` file *contains* passthrough metadata that an external program reading `.cs` directly otherwise would not have) (`official cryoSPARC documentation`, `official cryoSPARC documentation`) |
| Move a whole project | Export the project — see the data-management guide referenced by `official cryoSPARC documentation` and `14_cli_admin.md` |
| Bulk-download a list of jobs / projects / workspaces / sessions as CSV | GUI CSV download; respects filters and sort (`official cryoSPARC documentation`) |

### Destructive-change avoidance

- **Treat existing jobs and their files as read-only by default.** A tools script should produce *new* External Job outputs rather than overwriting files inside an existing job's directory; the GUI's data-cleanup tools and `delete_project` are the supported destruction paths (`official cryoSPARC documentation`).
- **Never remove a project directory from the filesystem by hand.** Project directories managed by attached CryoSPARC projects must be removed via the *Delete Project* GUI action or the `delete_project()` CLI method (`official cryoSPARC documentation`).
...
External Jobs respect the same input/output-group model as built-in jobs — they declare inputs, optionally declare expected output slots, and finalize when their script signals completion (`official cryoSPARC documentation`, `official cryoSPARC documentation`).
...
| `Project P# not found` / `Workspace W# not found` / `Job J# not found` | Project/workspace/job UID wrong, or the script's user does not have access | List the user's projects via tools; verify GUI access for the same user account | Use the correct UID; share the project with the automation user via the GUI's *Share With Users* (`official cryoSPARC documentation`) |
| Loading workspace or session object errors out on older instances | Pre-v4.6 bug in tools loading workspace/session objects | Master/tools version | Update master to v4.6+ (`public cryoSPARC release notes v4.6`) |
...
| `alignments3D_multi` missing or mismatched class count in downstream job | v5.0 fix changed semantics: when multiple particle set inputs are connected, `alignments3D_multi` is **removed** rather than passed through, to avoid class-count mismatch failures (`public cryoSPARC release notes v5.0`) | Master version | Update to v5.0+; restructure the input wiring so each connected set has a coherent class count |
...
| Unexpected `Permission denied` on NFS / Lustre even though the file is accessible | Listed UNIX permissions don't match the true permissions on the network filesystem | Try the path manually as the cryoSPARC owner | v5.0+ supports `CRYOSPARC_CLI_SKIP_ACCESS_CHECK=true` to disable that check at the instance level — set with care (`public cryoSPARC release notes v5.0`) |
...
| Workflow apply succeeds in GUI but fails when driven by script | Workflow-connection edge case (fixed in v5.0) or stale workflow JSON | Master version; workflow JSON checksum | Update master; re-export and re-import the workflow (`public cryoSPARC release notes v5.0`) |
...
- GUI job building and lifecycle: `official cryoSPARC documentation`
- Inspecting jobs (dashboard, charts, related jobs, comparisons): `official cryoSPARC documentation`
- Output groups and the Low Level Results Interface (passthroughs, slot swapping): `official cryoSPARC documentation`
- Projects, workspaces, Live sessions, sharing, deletion: `official cryoSPARC documentation`
- Downloading and exporting (jobs, projects, CSVs, event logs, job reports): `official cryoSPARC documentation`
- Workflow templates: `official cryoSPARC documentation`
- End-to-end automation (GPCR repeat-target template): `official cryoSPARC documentation`
- Library overview and links: `official cryoSPARC documentation`
