...
In local operator's rollout, `--permission-mode bypassPermissions` is deliberately used for all headless subprocess calls so Claude CLI never stalls on internal permission UI. That moves the safety boundary to the **parent agent**: approved plan, explicit cwd/worktree/sandbox, budget, timeout, `--effort max`, logs, and user feedback handling.
...
Functionally equivalent blast radius to `--dangerously-skip-permissions`, but it is the default for local operator's headless agent rollout to avoid approval deadlocks.
...
For agent runners in CI/containers, set `ANTHROPIC_API_KEY` explicitly. Don't rely on the host user's interactive login. For local OpenClaw agents using local operator's Claude Max / claude.ai login, omit `--bare` unless an API key/helper has been configured for that agent.
